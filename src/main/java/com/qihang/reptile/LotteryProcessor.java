        ...
        if (ObjectUtil.equal(page.getUrl().toString(), CrawlingAddressConstant.URL1)) {
            ...
            for (int i = 1; i <= nodes.size(); i++) {
                ...
                for (int j = 0; j < tr.size(); j++) {
                    ...
                    //
                    String halfWholeOdds= StrUtil.join(",",html.xpath("*[@id=\"relativeContainer\"]/div/div[3]/table[" + i + "]/tbody/tr[" + index + "]/td/div/table[1]/tbody/tr/td/p/i/text()").all()).replaceAll(",↑", "").replaceAll(",↓", "");
                    footballMatch.setHalfWholeOdds(halfWholeOdds);
                   //
                    String scoreOdds= StrUtil.join(",",html.xpath("*[@id=\"relativeContainer\"]/div/div[3]/table[" + i + "]/tbody/tr[" + index + "]/td/div/table[2]/tbody/tr/td/p/i/text()").all()).replaceAll(",↑", "").replaceAll(",↓", "");
                    footballMatch.setScoreOdds(scoreOdds);

                    String goalOdds= StrUtil.join(",",html.xpath("*[@id=\"relativeContainer\"]/div/div[3]/table[" + i + "]/tbody/tr[" + index + "]/td/div/table[3]/tbody/tr/td/p/i/text()").all()).replaceAll(",↑", "").replaceAll(",↓", "");
                    footballMatch.setGoalOdds(goalOdds);
