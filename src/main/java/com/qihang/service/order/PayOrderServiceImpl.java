...
public class PayOrderServiceImpl extends ServiceImpl<PayOrderMapper, PayOrderDO> implements IPayOrderService {
    ...
    public CommonListVO<PayOrderQueryVO> getAdminPayOrderPage(PayOrderQueryDTO payOrderQuery) {
        ...
        if (sysUserDO.getRoleName().equals("超级管理员") && sysUserDO.getRoleId() == 2) {
            ...
            qw.eq(StrUtil.isNotBlank(payOrderQuery.getTenantId()),PayOrderDO::getTenantId, payOrderQuery.getTenantId());
            Date startTime = null;
            Date endTime = null;
            if (ObjectUtil.isNotNull(payOrderQuery.getStartTime())) {
                startTime = DateUtil.parse(new SimpleDateFormat("yyyy-MM-dd").format(payOrderQuery.getStartTime()) + " 00:00:00");
                qw.ge(PayOrderDO::getCreateTime, startTime);
            }
            if (ObjectUtil.isNotNull(payOrderQuery.getEndTime())) {
                endTime = DateUtil.parse(new SimpleDateFormat("yyyy-MM-dd").format(payOrderQuery.getEndTime()) + " 23:59:59");
            ...
            for (PayOrderDO payOrder : payOrderPage.getRecords()) {
                ...
                payOrderQueryVO.setPhone(userMapper.selectById(payOrder.getUserId()).getPhone());
            ...
            //分页
            Page<PayOrderDO> page = new Page<>(payOrderQuery.getPageNo(), payOrderQuery.getPageSize());
            LambdaQueryWrapper<PayOrderDO> qw = new QueryWrapper<PayOrderDO>().lambda().eq(PayOrderDO::getTenantId,tid);
            //动态拼接查询条件
