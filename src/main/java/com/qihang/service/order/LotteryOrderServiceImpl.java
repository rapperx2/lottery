...
public class LotteryOrderServiceImpl extends ServiceImpl<LotteryOrderMapper, LotteryOrderDO> implements ILotteryOrderService {
    ...
    public CommonListVO<LotteryOrderVO> getLotteryOrderPage(LotteryOrderDTO lotteryOrder, Integer userId) {
        ...
        for (LotteryOrderVO lotteryOrderVO : orderList) {
            ...
            BallGameDO ballGame = ballGameMapper.selectOne(new QueryWrapper<BallGameDO>().lambda().eq(BallGameDO::getName, lotteryEnum.getValue()).eq(BallGameDO::getTenantId,1));
    ...
    @Override
    public BaseVO updatepic(LotteryOrderadminDTO lotteryOrderpic) {
        LotteryOrderDO lotteryOrder = lotteryOrderMapper.selectById(lotteryOrderpic.getOrderId());
        lotteryOrder.setPic(lotteryOrderpic.getPic());
    ...
    @Override
    public CommonListVO<LotteryOrderQueryVO> getAdminLotteryOrderPage(LotteryOrderQueryDTO lotteryOrderQuery, int tid) {
        ...
        if (sysUserDO.getRoleName().equals("超级管理员") && sysUserDO.getRoleId() == 2) {
            ...
            for (LotteryOrderDO lotteryOrder : lotteryOrderPage.getRecords()) {
                ...
                lotteryOrderQueryVO.setPhone(userDO.getPhone());
                //上级
            ...
            qw.eq((LotteryOrderDO::getTenantId),tid);
            qw.eq(StrUtil.isNotBlank(lotteryOrderQuery.getOrderId()), LotteryOrderDO::getOrderId, lotteryOrderQuery.getOrderId());
            qw.eq(StrUtil.isNotBlank(lotteryOrderQuery.getState()), LotteryOrderDO::getState, lotteryOrderQuery.getState());
            qw.eq(StrUtil.isNotBlank(lotteryOrderQuery.getType()), LotteryOrderDO::getType, lotteryOrderQuery.getType());
            qw.orderByDesc(LotteryOrderDO::getCreateTime);
...
//                .le(LotteryOrderDO::getUpdateTime, end)
    ...
    @Override
    public CommonListVO<LotteryOrderVO> getShopLotteryOrderPage(LotteryOrderDTO lotteryOrder) {
        System.out.println("======lotteryOrder====="+lotteryOrder);
        ...
        //根据手机号查询用户id
        UserDO userId = userMapper.selectOne(new QueryWrapper<UserDO>().lambda().eq(UserDO::getPhone, lotteryOrder.getPhone()));
...
//        }
        ...
        //动态拼接查询条件
        if (lotteryOrder.getPhone() != null && lotteryOrder.getPhone() != "") {
            if (userId != null && userId.getId() > 0) {
                qw.eq(LotteryOrderDO::getUserId, userId.getId());
            }else {
                return commonList;
            }
        }
...
//        qw.eq(LotteryOrderDO::getUserId, userId);
        ...
        if (lotteryOrder.getType() != null && lotteryOrder.getType() != "") {
            qw.eq(LotteryOrderDO::getType, lotteryOrder.getType());
        }
        qw.orderByDesc(LotteryOrderDO::getCreateTime);
        Page<LotteryOrderDO> lotteryOrderPage = lotteryOrderMapper.selectPage(page, qw);
        //转换vo
        ...
        for (LotteryOrderVO lotteryOrderVO : orderList) {
            //设置订单对应的彩票名字和logo
            LotteryOrderTypeEnum lotteryEnum = LotteryOrderTypeEnum.valueOFS(lotteryOrderVO.getType());
            BallGameDO ballGame = ballGameMapper.selectOne(new QueryWrapper<BallGameDO>().lambda().eq(BallGameDO::getName, lotteryEnum.getValue()).eq(BallGameDO::getTenantId,tid));
            lotteryOrderVO.setBallName(ballGame.getName());
            lotteryOrderVO.setBallUrl(ballGame.getUrl());
        }
        commonList.setVoList(orderList);
