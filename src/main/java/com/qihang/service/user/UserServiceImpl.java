...
public class UserServiceImpl extends ServiceImpl<UserMapper, UserDO> implements IUserService {
    ...
    public BaseVO register(RegisterDTO registerDTO) {
        ...
        //注册设置默认头像
        userDO.setAvatar("https://mylottery20230909.s3.ap-northeast-1.amazonaws.com/cms/cms/userLogo2000x2000.png");
    ...
    public CommonListVO<UserQueryVO> pageList(UserQueryDTO userQueryDTO) {
        ...
        if (servletRequestAttributes.getRequest().getHeader("X-User") != null && servletRequestAttributes.getRequest().getHeader("X-User") != "") {
            ...
            if (sysUserDO.getRoleName().equals("超级管理员") && sysUserDO.getRoleId() == 2) {
                ...
                qw.like(StrUtil.isNotBlank(userQueryDTO.getPhone()), UserDO::getPhone, userQueryDTO.getPhone());
                qw.like(StrUtil.isNotBlank(userQueryDTO.getName()), UserDO::getName, userQueryDTO.getName());
                qw.eq(StrUtil.isNotBlank(userQueryDTO.getIdCard()), UserDO::getIdCard, userQueryDTO.getIdCard());
                qw.eq(StrUtil.isNotBlank(userQueryDTO.getIsAgent()), UserDO::getIsAgent, userQueryDTO.getIsAgent());
                Date startTime = null;
                Date endTime = null;
                if (ObjectUtil.isNotNull(userQueryDTO.getStartTime())) {
                    startTime = DateUtil.parse(new SimpleDateFormat("yyyy-MM-dd").format(userQueryDTO.getStartTime()) + " 00:00:00");
                    qw.ge(UserDO::getCreateTime, startTime);
                }
                if (ObjectUtil.isNotNull(userQueryDTO.getEndTime())) {
                    endTime = DateUtil.parse(new SimpleDateFormat("yyyy-MM-dd").format(userQueryDTO.getEndTime()) + " 23:59:59");
                    qw.le(UserDO::getCreateTime, endTime);
                }
                ...
                LambdaQueryWrapper<UserDO> qw = new QueryWrapper<UserDO>().lambda().eq(UserDO::getTenantId,tid);
                //动态拼接查询条件
                qw.like(StrUtil.isNotBlank(userQueryDTO.getNickname()), UserDO::getNickname, userQueryDTO.getNickname());
                qw.like(StrUtil.isNotBlank(userQueryDTO.getPhone()), UserDO::getPhone, userQueryDTO.getPhone());
                qw.like(StrUtil.isNotBlank(userQueryDTO.getName()), UserDO::getName, userQueryDTO.getName());
    ...
    public BaseVO recharge(UserRechargeDTO userRecharge, Integer id) {
        ...
        payOrder.setTenantId(tid);
        payOrderMapper.insert(payOrder);
        return new BaseVO();
    }
    @Override
    public BaseVO agentedit(UserAgentEditDTO agent) {
         UserDO user = userMapper.selectById(agent.getId());

        user.setIsAgent(agent.getIsAgent());
        userMapper.updateById(user);
        return new BaseVO();
    }
    ...
    public AdminStatisticsVO userStatistics(UserStatisticsDTO userStatistics) {
        ...
        if (sysUserDO.getRoleName().equals("超级管理员") && sysUserDO.getRoleId() == 2) {
            ...
            if (ObjectUtil.isNull(userStatistics.getStartTime())) {
                ...
                startTime = DateUtil.parse(new SimpleDateFormat("yyyy-MM-dd").format(userStatistics.getStartTime()) + " 00:00:00");
            }
            if (ObjectUtil.isNull(userStatistics.getEndTime())) {
                endTime = DateUtil.parse(DateUtil.today() + " 23:59:59");
            } else {
                endTime = DateUtil.parse(new SimpleDateFormat("yyyy-MM-dd").format(userStatistics.getEndTime()) + " 23:59:59");
            }
            List<UserDO> records = userPage.getRecords();
    ...
    @Override
    public BaseVO getUserById(UserQueryDTO userQueryDTO) {
        UserVO userVO = new UserVO();
        UserDO user = userMapper.selectById(userQueryDTO.getId());
        BeanUtils.copyProperties(user, userVO);
        return userVO;
    }
