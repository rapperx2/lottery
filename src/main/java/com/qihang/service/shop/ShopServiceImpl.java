...
public class ShopServiceImpl extends ServiceImpl<ShopMapper, ShopDO> implements IShopService {
    ...
    @Resource
    private rackingRecordsMapper rackingRecords;

    @Override
    @TenantIgnore
    public CommonListVO<AdminShopVO> shopadminList(AdminShopDTO adminShop) {
        ...
        qw.orderByDesc(ShopDO::getCreateTime);
    ...
    @TenantIgnore
    @Transactional(rollbackFor = Exception.class)
    public BaseVO shopedit(EditShopDTO addShop, Integer userId) {
        System.out.println("=========addShop======"+addShop);

        ShopDO shop = shopMapper.selectById(addShop.getId());
        if (addShop.getName() != "" && addShop.getName() != null) {
            shop.setName(addShop.getName());
        }
        if (addShop.getLogo() != "" && addShop.getLogo() != null) {
            shop.setLogo(addShop.getLogo());
        }
        if (addShop.getLine() != "" && addShop.getLine() != null) {
            shop.setLine(addShop.getLine());
        }

        shop.setBalance(addShop.getBalance());


        //BeanUtils.copyProperties(addShop, shop);
        shop.setUpdateTime(new Date());
        shopMapper.updateById(shop);
        if (addShop.getReline()=="1")
        {
            LogDO logdo=new LogDO();
            logdo.setUserId(1);
            logdo.setCreateTime(new Date());
            logdo.setTenantId(1);
            logdo.setNickname("后台管理员");
            logdo.setDescriptor("为店铺"+addShop.getName()+"充值余额："+addShop.getBalance()+"元");
            logdo.setPhone("admin");
    ...
    @TenantIgnore
    @Transactional(rollbackFor = Exception.class)
    public BaseVO shopadd(AddShopDTO addShop, Integer userId) {
        SysUserDO user = sysUserMapper.selectOne(new QueryWrapper<SysUserDO>().lambda().eq(SysUserDO::getUsername, addShop.getSysUsername()));
        if (ObjectUtil.isNotNull(user)) {
            return new BaseVO(false, ErrorCodeEnum.E090.getKey(), ErrorCodeEnum.E090.getValue());
        }
        ...
        userDO.setAvatar("https://img-blog.csdnimg.cn/142cf2dca89e456aa3fe3aacdf688363.jpeg");
        userMapper.insert(userDO);
        WirthdrawalconfigDO Wirthdrawalconfig = new WirthdrawalconfigDO();
        Wirthdrawalconfig.setShopId(shop.getId());
        Wirthdrawalconfig.setTenantId(1);
        wirthdrawalconfigMapper.insert(Wirthdrawalconfig);
        rackingRecordsDO recordsra = new rackingRecordsDO();
        recordsra.setTenantId(shop.getId());
        rackingRecords.insert(recordsra);
        return new BaseVO();
    }
    ...
    public BaseVO delete(Integer id) {
        ...
        wirthdrawalconfigMapper.delete(new QueryWrapper<WirthdrawalconfigDO>().lambda().eq(WirthdrawalconfigDO::getShopId, id));
...
//        }
    ...
    @Override
    public CommonListVO<AdminShopVO> shopadminnopageList() {
        CommonListVO<AdminShopVO> commonList = new CommonListVO<>();
        List<ShopDO> shopList = shopMapper.selectList(new QueryWrapper<ShopDO>().lambda());
        List<AdminShopVO> list = BeanUtil.copyToList(shopList,AdminShopVO.class);
        commonList.setVoList(list);
        return commonList;
    }
}
