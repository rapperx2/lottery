...
public class WithdrawalServiceImpl implements IWithdrawalService {
    ...
    public BaseVO examine(ExamineDTO examineDTO) {
        ...
        int tenantId = Integer.parseInt(servletRequestAttributes.getRequest().getHeader("X-Tenant-Id"));
        WithdrawalDO withdrawal = new WithdrawalDO();
    ...
    public CommonListVO<WithdrawalQueryVO> pageList(WithdrawalQueryDTO withdrawalQuery) {
        ...
        if (servletRequestAttributes.getRequest().getHeader("X-User") != null && servletRequestAttributes.getRequest().getHeader("X-User") != "") {
            ...
            if (sysUserDO.getRoleName().equals("超级管理员") && sysUserDO.getRoleId() == 2) {
                ...
                //分页
                Page<WithdrawalDO> page = new Page<>(withdrawalQuery.getPageNo(), withdrawalQuery.getPageSize());
                LambdaQueryWrapper<WithdrawalDO> qw = new QueryWrapper<WithdrawalDO>().lambda().eq(WithdrawalDO::getTenantId,tid);
                //动态拼接查询条件
                qw.eq(ObjectUtil.isNotNull(userId), WithdrawalDO::getUserId, userId);
                qw.eq(StrUtil.isNotBlank(withdrawalQuery.getType()), WithdrawalDO::getType, withdrawalQuery.getType());
                qw.eq(StrUtil.isNotBlank(withdrawalQuery.getState()), WithdrawalDO::getState, withdrawalQuery.getState());
    ...
    @Override
    public CommonListVO<WithdrawalQueryVO> userList(WithdrawalDTO withdrawalDTO) {
        CommonListVO<WithdrawalQueryVO> commonList = new CommonListVO<>();
        List<WithdrawalDO> withdrawalList = withdrawalMapper.selectList(new QueryWrapper<WithdrawalDO>().lambda().eq(WithdrawalDO::getId, withdrawalDTO.getId()));
        List<WithdrawalQueryVO> list = BeanUtil.copyToList(withdrawalList, WithdrawalQueryVO.class);
        commonList.setVoList(list);
