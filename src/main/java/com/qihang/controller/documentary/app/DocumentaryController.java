...
public class DocumentaryController {
    ...
    @GetMapping("/detailslist/{id}")
    @ApiOperation("根据用户id查询跟单信息接口")
    public DocumentaryDetailsVO documentaryDetailslist(@PathVariable("id") Integer id) {
    ...
    public DocumentaryDetailsVO documentaryDetails(@PathVariable("id") Integer id) {
        return documentaryService.documentaryDetailsAdmin(id);
    }

    @GetMapping("/by/{type}/{userid}")
    @ApiOperation("根据类型查询对应的跟单数据")
    public CommonListVO<DocumentaryByTypeVO> queryDocumentaryByType(@PathVariable("type") String type,@PathVariable("userid") Integer id) {

        return documentaryService.queryDocumentaryByType(type, id);
    }
    @GetMapping("/byadmin/{type}/{userid}")
    @ApiOperation("根据类型查询对应的跟单数据")
    public CommonListVO<DocumentaryByTypeVO> queryDocumentaryByTypeAdmin(@PathVariable("type") String type,@PathVariable("userid") Integer id) {

        return documentaryService.queryDocumentaryByTypeAdmin(type, id);
    }
    ...
    public DocumentaryByIdVO queryDocumentaryById(@PathVariable("id") Integer id, @PathVariable("userId") Integer userId) {
        return documentaryService.queryDocumentaryById(id, userId, userId);
    }
